...
class RssItem  {
    ...
    triggerOpenSetting() {
        $('[data-class="'+this.obj.attr('rss-item')+'"]').trigger('click');
    }
...
class RssElement  {
    ...
    setOptions(options, callback) {
        ...
        window.currentEditor.handleSelect();
    }
    ...
    addEvents() {
        ...
        element.obj.find('[rss-item]').on('click', function(e) {
            ...
            if(typeof(window.currentEditor.selected.rssItemFactory) !== 'undefined') {
            ...
            // rss element not selected then select
                ...
                var e = window.currentEditor.elementFactory(t);
                window.currentEditor.select(e);
                window.currentEditor.handleSelect(function() {
                    setTimeout(function() {
    ...
    setContent(content) {
        ...
        if (this.getOptions().preview == 'no') {
            button = `
                <button class="btn btn-secondary ace-rss-button ace-preview shadown-sm" style="display:none">Preview</button>
            `;
        } else {
            button = `
                <button class="btn btn-secondary ace-rss-button ace-unpreview shadown-sm" style="display:none">Unpreview</button>
            `;
        }
    ...
    loadPlaceholder() {
        ...
        _this.setContent(`
            <div class="rss-placeholder" style="text-align: center; display: flex; background-color: #666; padding: 40px 50px; color: #fff; border-radius: 5px;align-items:center;">
        ...
        //     window.currentEditor.select(_this);
        //     window.currentEditor.handleSelect();
        // };
    }
    ...
    loadRss(callback) {
        ...
        var url = _this.obj.attr('data-action');
    ...
    getControls() {
        ...
        return [
            new window.RssControl(window.getI18n('font_family'), element.getOptions(), {
            ...
            new window.FontFamilyControl(window.getI18n('font_family'), element.obj.css('font-family'), function(font_family) {
                element.obj.css('font-family', font_family);
                element.select();
            }),
            new window.BackgroundImageControl(window.getI18n('background_image'), {
            ...
            new window.BlockOptionControl(window.getI18n('block_options'), { padding: element.obj.css('padding'), top: element.obj.css('padding-top'), bottom: element.obj.css('padding-bottom'), right: element.obj.css('padding-right'), left: element.obj.css('padding-left') }, function(options) {
